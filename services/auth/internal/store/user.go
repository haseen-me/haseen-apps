...
func (s *Store) CreateUser(ctx context.Context, email, srpSalt, srpVerifier string) (*model.User, error) {
	...
	err := s.DB.QueryRow(ctx,
		`INSERT INTO users (email, srp_salt, srp_verifier)
		 VALUES ($1, $2, $3)
		 RETURNING id, email, srp_salt, srp_verifier, created_at, updated_at`,
		email, srpSalt, srpVerifier,
	).Scan(&u.ID, &u.Email, &u.SRPSalt, &u.SRPVerifier, &u.CreatedAt, &u.UpdatedAt)
	if err != nil {
		return nil, err
	}
	return u, nil
...
func (s *Store) GetUserByEmail(ctx context.Context, email string) (*model.User, error) {
	u := &model.User{}
	err := s.DB.QueryRow(ctx,
		`SELECT id, email, srp_salt, srp_verifier, created_at, updated_at
