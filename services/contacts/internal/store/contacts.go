...
`INSERT INTO contacts (user_id, email, name, notes)
 VALUES ($1, $2, $3, $4)
 ON CONFLICT (user_id, email) DO UPDATE SET name = EXCLUDED.name, notes = EXCLUDED.notes, updated_at = now()
 RETURNING id, email, name, notes, created_at, updated_at`,
userID, req.Email, req.Name, req.Notes,
).Scan(&c.ID, &c.Email, &c.Name, &c.Notes, &c.CreatedAt, &c.UpdatedAt)
return c, err
}
...
c := &model.Contact{}
err := s.DB.QueryRow(ctx,
`SELECT id, email, name, notes, created_at, updated_at
...
for rows.Next() {
var c model.Contact
if err := rows.Scan(&c.ID, &c.Email, &c.Name, &c.Notes, &c.CreatedAt, &c.UpdatedAt); err != nil {
return nil, err
}
...
_, err = s.DB.Exec(ctx,
`UPDATE contacts SET name = $3, email = $4, notes = $5, updated_at = $6
 WHERE id = $1 AND user_id = $2`,
contactID, userID, c.Name, c.Email, c.Notes, c.UpdatedAt,
)
return c, err
}
